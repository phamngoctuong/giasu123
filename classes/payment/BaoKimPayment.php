...
class BaoKimPayment 
{
    private $baokim_url  = "";
	// Mã merchante site 
	private $merchant_id = '';	// Biến này được baokim.vn cung cấp khi bạn đăng ký merchant site

	// Mật khẩu bảo mật
	private $secure_pass = ''; // Biến này được baokim.vn cung cấp khi bạn đăng ký merchant site
    ...
    public function __construct($baokim_url="https://www.baokim.vn/payment/customize_payment/order",$merchant_id,$secure_pass) {
		$this->baokim_url  = $baokim_url;
        $this->merchant_id = $merchant_id;
        $this->secure_pass = $secure_pass;
	}
    
	public function createRequestUrl($order_id, $business, $total_amount, $shipping_fee, $tax_fee, $bank_payment_method_id="", $order_description, $url_success, $url_cancel, $url_detail)
	{  
        ...
        if($bank_payment_method_id != ""){
			$params['bank_payment_method_id'] = $bank_payment_method_id;
        }
	 ...
	 */
	public function verifyResponseUrl($_GET)
	{
