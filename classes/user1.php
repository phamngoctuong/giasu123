...
class user{
	...
	*/
	function user($login_name="",$password="", $publisher = 0){
		...
		if ($login_name==""){
			if (isset($_COOKIE[$this->pre_cookie . "loginname"])) $login_name = $_COOKIE[$this->pre_cookie . "loginname"];
		}
		if ($password==""){
			if (isset($_COOKIE[$this->pre_cookie . "PHPSESSlD"])) $password = $_COOKIE[$this->pre_cookie . "PHPSESSlD"];
			$checkcookie=1;
		}
		else{
	...
	*/
	function savecookie($time=0){
		...
		foreach($this->arrayDomainCookie as $key => $domain){
			if ($time > 0){
				if($domain == ""){
					setcookie($this->pre_cookie . "loginname",$this->login_name,time()+$time,"/");
					setcookie($this->pre_cookie . "PHPSESSlD",$this->password,time()+$time,"/");
				}else{
					setcookie($this->pre_cookie . "loginname",$this->login_name,time()+$time,"/", $domain);
					setcookie($this->pre_cookie . "PHPSESSlD",$this->password,time()+$time,"/", $domain);
				}
				//setcookie("u_id",$this->u_id,time()+$time);
			}
			else{
				if($domain == ""){
					setcookie($this->pre_cookie . "loginname",$this->login_name,null,"/");
					setcookie($this->pre_cookie . "PHPSESSlD",$this->password,null,"/");
				}else{
					setcookie($this->pre_cookie . "loginname",$this->login_name,null,"/",$domain);
					setcookie($this->pre_cookie . "PHPSESSlD",$this->password,null,"/", $domain);
				}
...
//Thiết lập quyền cho " . $this->table_user . " đưa tin HOT
