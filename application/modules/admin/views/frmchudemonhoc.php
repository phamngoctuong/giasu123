...
<div class="content-inner1">
	<form name="frmdanhmuc" action="<?php echo site_url('admin/add_chudemonhoc'); ?>" method="post" enctype="multipart/form-data">
		<?php
			if(isset($id))
			{
				$this->db->where('ID',$id);
		...
		<table class="tab1">
    		<tr>
    			...
    			<td><input type="text" name="NameTopic" width="250" value="<?php if(isset($id)) {echo $item->NameTopic;} ?>" /></td>
    		</tr>
    		<tr class="second">
                    ...
                    <select id="SubjectID" name="SubjectID">
                        ...
                        <?php if(!empty($listdefault)){
                            foreach($listdefault as $i){
                                if($item->SubjectID == $i->ID && !empty($item)){
                        ?>
                            <option value="<?php echo $i->ID ?>" selected="selected"><?php echo $i->SubjectName ?></option>
                        <?php }else{ ?>
                            <option value="<?php echo $i->ID ?>"><?php echo $i->SubjectName ?></option>
                        <?php } ?>
                        <?php } } ?>
                    </select>
    			</td>
		...
		<div class="">
		  ...
		  <table width="30%" style="display:block;">
		    ...
		    <?php
		    $stt=1;
				$areaclass=explode(',',$item->idlophoc);
