...
<form name="frmsearch" method="post" action="<?php echo site_url('admin/doanhnghiep'); ?>">
...
<form name="frmxoaall" id="frmxoaall" method="post" action="<?php echo site_url('admin/del_doanhnghiep'); ?>">
<p class="sidebar"><input type="submit" name="submit" value="Xóa" /></p>
<table width="100%" style="display:block;">
    <tr class="title">
        <td width="3%" align="center"><input type="checkbox" onclick="checkall('checkbox', this)" name="check"/></td>
				<td width="5%" align="center">id</td>
				<td>Tên doanh nghiệp</td>
				<td width="20%">Địa chỉ</td>
				<td width="10%">Người đăng ký</td>
				<td width = "5%">Ngày đăng ký</td>
				<td width="6%">Số điện thoại</td>
				<td width="6%">Email</td>
        <td width="10%">Khoá/Mở tài khoản</td>
        <td width="10%">Thành phố</td>
    </tr>
	<?php
	if($query !='')
	{
    ...
    ?>
		<tr class="<?php echo $stt%2 ? 'odd' : 'even'; ?>">
			<td align="center">
			<div id="request-form">
				<input type="checkbox" name="checkbox[]" class="checkbox" value="<?php echo $item->usc_id; ?>" />
			</div>
			</td>
			<td><?php echo $item->usc_id?></td>
			<td><a ><?php echo $item->usc_company; ?></a></td>			<!--href="<?php echo site_url('admin/edit_doanhnghiep/'.$item->ClassID); ?>"-->
			<td><?php echo $item->usc_address;?></td>
			<td><?php echo $item->usc_name ; ?></td>
			<td><?php echo timenow($item->usc_create_time)?></td>
			<td> <?php echo $item->usc_phone ?> </td>
			<td> <?php echo $item->usc_email ?> </td>
			<td align="center">
					<?php
					if($item->usc_active=='0')
					{
					?>

					<a class="status" onclick="check_statusck('user_company','usc_active',1,'usc_id',<?php echo $item->usc_id; ?>)"><img src="images/toolbar/tick.png"></a>
					<?php
					}
					else
					{
					?>
					<a class="status" onclick="check_statusck('user_company','usc_active',0,'usc_id',<?php echo $item->usc_id; ?>)"><img src="images/toolbar/publish_x.png"></a>
					<?php
					}
					?>
				</td>

			<td><?php echo Getcitybyindex($item->usc_city);?></td>
