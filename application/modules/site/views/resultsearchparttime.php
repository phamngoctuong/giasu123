...
<section class="inner-header-title padd-top-20 padd-bot-20" style="background-image:url(images/banner-10.jpg);">
	<div class="container">
		<div class="row">
            <div class="col-md-12"><h1 class="padd-0 mrg-0 text-center font-30">Tìm kiếm việc làm thêm</h1></div>
                <form class="frmparttime">
					<div class="col-md-4 col-sm-4">
									<input value="<?php if(!empty($fillkey['key'])&&$fillkey['key']!='all' ){ echo $fillkey['key'];} ?>" class="form-control" id="findparttime" placeholder="Nhập tên công việc muốn tìm" type="text">
					</div>
                    <div class="col-md-2 col-sm-2">
                                    <select id="txtcalamviec" name="txtcalamviec" class="form-control">
                                        <option value="">Chọn ca làm việc</option>
                                        <option <?php if(intval($fillkey['ca'])==1){ echo 'selected="selected"'; } ?> value="1">Sáng (từ 8h đến 14h)</option>
                                        <option <?php if(intval($fillkey['ca'])==2){ echo 'selected="selected"'; } ?>  value="2">Chiều (Từ 14h đến 21h)</option>
                                    </select>
                    </div>
								<div class="col-md-2 col-sm-2">
...
<section class="padd-top-0">
    <div class="container">
        <div class="row">
            ...
            <div class="vl_lc">
                <div class="col-md-12"><div class="tit_hd">
                    <div class="ir_h3">
                       <h3><span>Kết quả tìm kiếm việc làm thêm</span></h3>
                    </div>
                    <a href="<?php echo site_url(); ?>tim-viec-lam-them&key=all&c=0&n=0&t=0" class="span_hd">Xem tất cả <img src="images/ic_muiten.png" alt="#"/></a>
                 </div>
                 </div>
                <div class="col-md-6 col-sm-12 padd-r-5 vlthemmoi">
                    ...
                    <?php
                        if(!empty($parttime)){ $i=0; 
                            foreach($parttime as $n){ if($i<(count($parttime)/2)){ ?>
                                <div class="itemnews">
                ...
                <div class="pagation col-md-12 col-sm-12 mrg-top-20">
          						<?php echo $pagination; ?>
           		</div>         
            </div>
        </div>
        ...
        <!--<div class="row">
            <div class="col-md-12">
                <div class="tit_hd">
                 <div class="ir_h3"><h3><span>Việc làm thêm tiêu biểu</span></h3>
                 </div>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="tit_hd">
                 <div class="ir_h3"><h3><span>Việc làm fulltime mới nhất</span></h3>
                 </div>
                 <a href="#" class="span_hd">Xem tất cả <img src="images/ic_muiten.png" alt="#"/></a>
                </div>
                <div class="">
...
<script type="text/javascript">
	$(document).ready(function() {
	   $('#txtnnpartime').val(<?php echo $fillkey['nganh']; ?>).select2();
       $('#txtcityparttim').val(<?php echo $fillkey['tinh']; ?>).select2();
       $('#txtcalamviec').select2()
	    var configulr='<?php echo site_url() ?>';
        $('.timvieclampartime').on('click',function(){
            var key=$('#findparttime').val();
            var ca=$('#txtcalamviec').val();
            var nganh=$('#txtnnpartime').val();
            var tinh=$('#txtcityparttim').val();
            if(key !='' || ca !='' || nganh !='' || tinh !=''){
                $.ajax(
              {
                  
                  url: configulr+"site/ajaxsearchparttime",
                  type: "POST",
                  data: { key:key,ca:ca,nganh:nganh,tinh:tinh },
                  dataType: 'json',
                  beforeSend: function () {
                      $("#boxLoading").show();
                  },
