                ...
                <div class="listnganhnghe">
                    <ul>
                         <?php
                                    if(!empty($monhoc)){
                                        foreach($monhoc as $n){ ?>
                                            <li class="col-md-4 padd-0"><a target="_blank" href="<?php echo base_url() ?>gia-su&key=all&subject=<?php echo $n->ID ?>&topic=0&place=0&type=0&sex=0"><?php echo $n->SubjectName ?></a></li>
                                        <?php }
                                    }
                         ?>
                    </ul>
	...
	$(document).ready(function() {
	   ...
	   $("#findkey").keypress(function (e) {
        if (e.which === 13) {
            ...
            $.ajax(
              {
                url: configulr+"site/ajaxfindsubject",
                  type: "POST",
